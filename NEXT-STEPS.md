...
| **Pipeline** | Green: 228 tests, typecheck, lint, format, real `wrangler deploy --dry-run` against the live account |
...
| `docs/` | Decision log at D-015, lessons journal live, both beside the set. |
...
| `apps/worker/src/reducer.ts` | Done for all four chambers, plus the two PILOT actions (`grip_bar`, `release_bar`) the finale needs. |
...
### 2. The Archive beat [blocks full-mode completion]

**A real gap, found by tracing the full chain.** Solving `blind_panel` moves the machine to `ARCHIVE`, not `TRANSITIONING`, because the Archive sits between Chambers II and III (doc 02 section 4). Nothing fires `ARCHIVE_COMPLETE`, so **full mode currently dead-ends after Chamber II**. BRIEF mode is unaffected (it skips `blind_panel` entirely) and does reach the finale, which is how Chamber III is tested today.

Doc 02 section 4 says reading the ghost log is required to progress, so the fix is the real feature, not a skip:

- `read_station_log({ entry })` as the action that completes the beat.
- The ghost fixtures are JSONL in **our own `SessionEvent` format**, so they can be generated by actually playing a session through the reducer and recording it, rather than hand-written. A session that ends in DEADLOCK is exactly the "they didn't make it either" ghost doc 02 wants, and it makes the "the benchmark corpus is the game's archive" claim literally true rather than aspirational.
- This is also where `apps/archive` (the cross-origin tool provider) becomes real, so the spike's `crossorigin.delegation` result matters here.

### 3. The server-authoritative timer

Still nothing fires `TIMER_EXPIRED`. Needs a Durable Object alarm (`state.storage.setAlarm`), not a client-driven tick, since the timer must be tamper-proof (doc 05 section 1). Chamber II's gauge drift (doc 02 section 3.3) is the other thing waiting on it.

Note that Chamber III did **not** need this: its stamina window and lockout are derived from timestamps passed into the pure reducer, so the whole finale is testable without any alarm. Prefer that pattern where it fits.
...
- **Check a chamber's secret against what an adversary would actually try, not just against its own rules** (D-014). Chamber III's design doc used an English passphrase, which meant exactly one of its 26 decryptions was readable and an agent solved the finale alone: 0 bits, not the published 4.70. Every chamber's secret space is small enough to enumerate; enumerate it and look for a distinguishable member before trusting the bits figure.
