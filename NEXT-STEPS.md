...
| **Judge path** | **Built** (D-058, D-059). Doc 08 phase 4 is closed. SPECTATE on the gate screen, attract mode after twenty seconds on the landing screen, the ablation folded into the start card, and `?chamber=N` deep links that walk the real transitions rather than assigning a state. All three recorded-session surfaces share one painter, `render/monitor.ts`, which uses a 2D context so the gate never fetches Three.js. |
...
replay viewer)** are built and verified in Chrome (D-056 to D-061). Phase 4's
last unchecked line, "starter prompt card polished to final art", is an art task
