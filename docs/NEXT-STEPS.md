...
| `apps/game/src/audio/*` | **New** (D-050, D-051). `plan.ts` is pure and tested; `engine.ts` owns the only `AudioContext`; `voices.ts` synthesises everything, including the theme's note tables; `index.ts` schedules. |
