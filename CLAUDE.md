...
| [apps/game/](apps/game/) | Phaser client. Renders PILOT's view, hosts the WebMCP tool director. | Complete through plan 1.4: the tool layer, the view feed, and Phaser with both scenes. All four rooms render greybox; the notepad is the gap. |
| [apps/worker/](apps/worker/) | Cloudflare Worker plus the Session Durable Object. Authoritative state. | Complete: four chambers, the reducer, the Archive beat, the server-authoritative timer, the read-only tool surface, the manual, PILOT's view socket and the CONCORD route |
