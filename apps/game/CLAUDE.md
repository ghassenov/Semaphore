...
- **A door stands in a hole, and when it cannot, the corridor moves** (D-053). `doorways.ts` says which wall of a room has a gap in it and `doorways.test.ts` derives the real gaps from `stationCells` and refuses any door that is not in one. Two openings could not carry a door - a south face, and eleven metres of gauge bank - and the answer was to reroute the corridors, not to bend the doors: **the building's shape is the cheapest thing in the model to change and it is the last thing anybody thinks of changing.**
- **Walking back through a door is a camera feature and may never become anything else** (D-054). Nothing crosses the wire, the chamber clock keeps running, and `doorLeadsTo` is the whole gate: the door is open, it leads somewhere in play order, and the room beyond is one the pair has already stood in. A room ahead of them has never been drawn. A change that lets PILOT reach a projection the server has not already pushed is the one class of change that is never accepted.
- **A caption on a side wall can only be separated from another one vertically.** A wall running away from the camera barely moves across the frame, so three metres of separation in the room is a dozen pixels on screen (D-054). Captions are checked in metres *and* by projection through the real camera; the anchor check alone has let this through once.
...
- **PILOT's poses come from what the stage already knows** (D-052): the stride, the fixture `E` is holding, the room's own solved flag. A pose the renderer has to be *notified* of is a pose that can disagree with the room. They ease rather than snap, like every other device (D-037).
- **A body at rest must not be a body at zero.** Every walk term multiplies by speed, so a standing figure was perfectly still, which at forty pixels tall reads as a rendering fault rather than as a person waiting. The breath cycle is worth more than the other three poses combined.
...
- **The palette was never what made the page feel flat** (D-052). Depth, layering, falloff and motion were. Before proposing a colour change, check that the thing being fixed is actually a colour: the twenty-colour set is carefully argued (D-043) and the channel hues carry the design law and the colourblind guarantee.
- **Dressing may move, and it still has no state** (D-055). Nothing in it is read, nothing survives a repaint, and the sway is driven off each piece's own position rather than a stored phase - which is what keeps "built once per room and never touched again" true while the room stops being a photograph.
- **Every animation sits behind `prefers-reduced-motion`**, and the reduced path must still be a working interface rather than a frozen one. PILOT's body is called under it with the stride zeroed rather than not called at all, because a body that breathes is not vestibular motion and freezing it left those players a mannequin.
...
### Access, and the replay

- **The mirror is the one sanctioned exception to the no-puzzle-values-in-DOM rule, and it is a trade-off rather than a loophole** (D-061). It is off by default, turned on by the person it is for, in a panel that says what it does first; and it **never names a glyph** - it says a plate carries a mark and leaves the describing to the player, exactly as the picture does. `mirror.test.ts` asserts that for every chamber, matching whole words rather than substrings, because `cross` is inside `across` and loosening what counts as a leak is the one change never accepted.
- **A contrast mode derives from the palette; it never declares a colour.** `check-palette.mjs` holds every `--name: #rrggbb` in `style.css` to `palette.ts` and rejected the first attempt, correctly. Deriving with `color-mix` is also the better answer: the channel hues carry the design law and the colourblind guarantee (D-043), and a contrast mode that quietly re-hued them would break the thing its own users most need to stay stable.
- **Reduced motion is read every frame, not captured at boot** (D-061). The Access switch can turn it on mid-session, and a value read once in the constructor cannot hear it. Anything keyed on stillness - the drift, the stride, the dust - is decided inside the loop.
- **The gate draws the recording twice, and only the gate** (D-083). `paintKeeperMonitor` is `paintMonitor`'s twin: the same second as KEEPER perceived it, which is the calls it made and a dashed hole where the room would be. **Both bars measure against the same `durationMs`** - the first build divided the cyan one by the last call's timestamp, the two sat visibly apart, and the picture quietly said these were not the same moment, which is the one thing it exists to say they are. `/ghost` is the only route in the repository that carries both projections, and it is safe only because nobody is playing behind it.
- **The caption band says one thing from inside a room, and that is the Blackout** (D-084). Everything else it prints is a phase with no room. The lamps failing has to be on the room, because a player watching the lights go out, the gauge bank stop being drawn and their agent start describing needles it has never seen is a player who cannot tell a beat from a bug - and the only other statement of it lives in a drawer that is closed by default.
- **One recorded-session picture, three surfaces** (D-058). `render/monitor.ts` is what the Archive's CRT plays, what SPECTATE plays on the gate and what the replay viewer draws. It uses a 2D context and nothing else, and that is load-bearing: nothing on its path may import Three.js, or the gate screen starts fetching the 143KB engine it exists to spare that browser.
- **`base: "./"` and a nested route are incompatible** (D-060). Relative asset references resolve against the current URL's *directory*, so a page at `/replay/abc` asks for `/replay/assets/index-*.js` and comes up blank - in production only, because Vite serves an absolute `/src/main.ts` in development. The replay's URL is `/replay?id=...` for this reason and because the worker's API answered on the path form with a `cache-control` header, so a navigation was served the cached JSON instead of the app.
- **A prop is not an error.** A monitor reading `NO TAPE` is what a null recording is *meant* to look like, so a failed fetch produced a screen that reported nothing wrong and a check asking "is anything lit" passed on it. A check for a rendered thing has to name the thing, not its brightness.

- **The starter prompt card is built once and rendered twice, and that is a rule** (D-062). It is on the never-cut list, and as two hand-assembled copies it had already drifted: the gate screen's lost the fallback line doc 04 section 2 asks for by name, which is the recovery path for the exact failure the card exists to prevent. The tour asserts the card is *whole and on screen*, not that an element with that class exists.
- **A panel that opens itself must hand the room back.** The slip opens on load because it is the most important thing on the landing screen, and closes when the shift starts, once, and only if it is still the panel that was opened. The room is the page (D-052): a drawer overlaying its right third during play is the console talking over the game, and one that reopened itself would be the console overruling the player.
...
- **The tour waits on `data-settled`, which the stage sets, and never on a copied constant** (D-056). A frame grabbed early is the previous room with the next room's name over it, or the whole building from four hundred metres up. Both have happened, once per renderer, and the last time the tour's own header stated the rule it was breaking: the Archive frame was taken at 2000ms against a 2400ms arrival and twenty-one assertions stayed green. A number copied out of another module has no way to hear that it changed.
...
| 2026-08-30 | Ahmed Saad | The console was rebuilt around the room (D-052): a Console section here, plus the rules that a drawer never pushes the viewport, that `[hidden]` needs help against `display`, that the palette was not what made the page flat, and that a body at rest must not be a body at zero. |
...
| 2026-09-01 | Ahmed Saad | The Blackout's client half and the split replay (D-081 to D-084): the room goes dark and PILOT gets the panel, `rotate_dial` leaves the registry mid-chamber, the gate draws the ghost twice, and the audio layer became a place (see `src/audio/CLAUDE.md`). |
